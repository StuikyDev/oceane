(function(){if(window.tagjsNetreviews!==undefined)return;window.tagjsNetreviews={};window.tagjsNetreviews.averageCalled=false;window.tagjsNetreviews.nrPlatform='br';window.tagjsNetreviews.nrIdWebsite='969384d1-eb8a-64e4-a1f2-c46b29a4d241';window.tagjsNetreviews.nrUrl="https://cl.avis-verifies.com/"+window.tagjsNetreviews.nrPlatform+"/cache/"+window.tagjsNetreviews.nrIdWebsite[0]+"/"+window.tagjsNetreviews.nrIdWebsite[1]+"/"+window.tagjsNetreviews.nrIdWebsite[2]+"/"+window.tagjsNetreviews.nrIdWebsite+"/AWS/";if(typeof(jQuery)=='undefined'&&typeof($)=='undefined'){/*! jQuery v1.8.2 jquery.com | jquery.org/license */
...
if(typeof jQuery!=='undefined'){$tagjs=jQuery;jQuery(document).ready(function(){nrIncludeCss();if($tagjs('.NETREVIEWS_PRODUCT_STARS').length>0){nrSearchForStars();}
if($tagjs('.NETREVIEWS_PRODUCT_REVIEWS').length>0){nrSearchForReviews();}});}else{return;}})();function textRemoval(){var detectNoTextClass=document.getElementsByClassName('no_text');var detectText=document.getElementsByClassName('netreviews_reviews_rate');for(var i=0;i<detectNoTextClass.length;i++){for(var j=0;j<detectText.length;j++){if(detectNoTextClass[i]===detectText[j].parentNode.parentNode){detectText[j].style.display='none';break;}}}}
function nrSearchForStars(){var variants=[];var allProducts=[];$tagjs('.NETREVIEWS_PRODUCT_STARS').each(function(){if($tagjs(this).attr('data-product-id')!==undefined&&allProducts.indexOf($tagjs(this).attr('data-product-id'))===-1){var products=$tagjs(this).attr('data-product-id');if(products.indexOf(',')>-1){variants=products.split(',');variants.forEach(function(item){if(allProducts.indexOf(item)===-1){allProducts.push(item);}})}else{allProducts.push(products);}}});var bodyParams={query:"average",products:allProducts,idWebsite:window.tagjsNetreviews.nrIdWebsite,plateforme:window.tagjsNetreviews.nrPlatform};$tagjs.ajax({type:"POST",data:JSON.stringify(bodyParams),url:"https://awsapis3.netreviews.eu/product",contentType:"application/json; charset=utf-8"}).done(function(data){if(data){for(var o in data){if(data[o]!=''&&typeof data[o]!=="undefined"){$tagjs('.NETREVIEWS_PRODUCT_STARS').each(function(){var array=[];if($tagjs(this).attr('data-product-id').indexOf(',')>-1){array=$tagjs(this).attr('data-product-id').split(',');}
...
createStarsBlock(average,index,vm);}}else if(o==$tagjs(this).attr('data-product-id')){createStarsBlock(data,o,this);}})}}}})}
function createStarsBlock(data,o,that){var result=null;if(typeof data[o]==="undefined"){result=data;}else{result=data[o][0];}
var nrRate=parseFloat(result.rate.toString());var nrReviews=result.count;var html='';if(nrRate!==0&&isNaN(nrRate)!==true){nrRate=nrRate.toPrecision(2);html+='<div class="netreviews_review_rate_and_stars">'
+'    <div>';for(var j=0;j<5;j++){html+='<span class="nr-icon nr-star grey"></span>';}
html+='  </div>'
+'    <div>';for(var k=1;k<=5;k++){if(nrRate>k){var starWidth='width:20%;';}else{var tempWidth=(nrRate-(k-1))*20<0?"0":(nrRate-(k-1))*20;starWidth='width:'+Math.round(tempWidth)+'%;';}
html+='<span class="nr-icon nr-star gold" style="'+starWidth+'"></span>';}
html+='  </div>'
+'    <div class="netreviews_reviews_rate">'+nrRate+'/5 - '+nrReviews+' Opiniões</div>'
+'</div>';$tagjs(that).php(html);textRemoval();}}
function nrSearchForReviews(){var allReviewsProducts=null;var products=$tagjs('.NETREVIEWS_PRODUCT_REVIEWS').attr('data-product-id');if(products.indexOf(',')>-1){allReviewsProducts=[];variants=products.split(',');variants.forEach(function(item){if(allReviewsProducts.indexOf(item)===-1){allReviewsProducts.push(item);}})}else{allReviewsProducts=products;}
var bodyParams={query:"reviews",products:allReviewsProducts,idWebsite:window.tagjsNetreviews.nrIdWebsite,plateforme:window.tagjsNetreviews.nrPlatform,offset:0,limit:5};$tagjs.ajax({type:"POST",data:JSON.stringify(bodyParams),url:"https://awsapis3.netreviews.eu/product",contentType:"application/json; charset=utf-8"}).done(function(data){var html='';var richSnippetsAggregateRating='';if(data){if(data.length>0){var nbReviews=data[0].stats;var reviewsTotalNb=nbReviews[0]+nbReviews[1]+nbReviews[2]+nbReviews[3]+nbReviews[4];var average=Math.round((nbReviews[0]+2*nbReviews[1]+3*nbReviews[2]+4*nbReviews[3]+5*nbReviews[4])/ reviewsTotalNb*10)/ 10;var dataProps=getDataProps();richSnippetsAggregateRating='<script type="application/ld+json">'
+'    {'
+'        "@context": "http://schema.org",'
+'        "@type":"Product",'
+'        "@id":"'+dataProps.product_rs_id+'",'
+'        "name":"'+dataProps.product_rs_name+'",'
+'        "image": "'+dataProps.product_rs_image+'", '
+'        "aggregateRating":{'
+'            "@type":"AggregateRating",'
+'            "ratingValue":"'+average+'",'
+'            "ratingCount":"'+reviewsTotalNb+'",'
+'            "bestRating":"5",'
+'            "worstRating":"1"'
+'        },'
+'        "offers":{'
...
+'            "price": "'+dataProps.product_price+'",'
+'            "priceCurrency": "BRL",'
+'            "url": "'+dataProps.product_rs_url+'"'
+'        }'
+'    }'
+' </script>';html+='<div id="netreviews_reviews_tab">'
+'     <input type="hidden" id="netreviews_template" value="1">'
+'     <div id="netreviews_rating_section">'
+'         <div class="netreviews_rating_header">'
+'             <div class="netreviews_logo"></div>'
+'             <div>'
+'                 <a class="netreviews_certification" target="_blank" href="https://www.opinioes-verificadas.com.br/opinioes-clientes/oceane.com.br">Veja nosso Selo de Confiança</a><br>'
+'             </div>'
+'         </div>'
+' <div class="netreviews_rating_content"';html+='>';html+='<div class="netreviews_global_rating">'
+'                 <p class="netreviews_note_generale">'
+'                     '+average+'<span>&nbsp;/5</span>'
+'                 </p>'
+'                 <div class="netreviews_bg_stars_big">'
+'                     <div>';for(var j=1;j<=5;j++){html+='<span class="nr-icon nr-star grey"></span>';}
html+='</div>'
+'<div>';for(var k=1;k<=5;k++){var starWidth='';if(average>k){starWidth='width:20%;';}else{var tempWidth=((Math.round(average*10)/ 10-(k-1))*20<0?"0":(Math.round(average*10)/ 10-(k-1))*20);starWidth='width:'+Math.round(tempWidth)+'%;';}
html+='<span class="nr-icon nr-star gold" style="'+starWidth+'"></span>';}
var str='Calculado de %s revisão (ões) de clientes';str=str.replace('%s',reviewsTotalNb);html+='                     </div>'
+'                 </div>'
...
+'                 <option value="newest" selected="selected">Mais recentes</option>'
+'                 <option value="oldest">O mais antigo</option>'
+'                 <option value="highest">Notas mais altas</option>'
+'                 <option value="lowest">Notas mais baixas</option>'
+'                 <option value="most_useful">O mais útil</option>'
+'             </select>'
+'         </div>'
+'     </div>'
...
+'         <a id="netreviews_media_close" href="#">×</a>'
+'     </div>'
+'     <div id="rich_snippets_json_ld">'
+'     </div>'
+'     <input type="hidden" id="netreviews-idWebsite" value="'+window.tagjsNetreviews.nrIdWebsite+'"/>'
+' </div>';$tagjs('.NETREVIEWS_PRODUCT_REVIEWS').php(html);$tagjs("#rich_snippets_json_ld").append(richSnippetsAggregateRating);if($tagjs('#netreviews_reviews_filter').length>0){$tagjs('#netreviews_reviews_filter').change(function(){$tagjs('#netreviews_review_list').php('');$tagjs('#avisVerifiesAjaxImage').show();nrGetReviews([1,2,3,4,5],0,5,reviewsTotalNb);});}
...
+'<span class="nr-icon nr-star grey"></span>'
+'        </div>'
+'        <div>';for(var j=1;j<=5;j++){var starWidth='';if(data[i].rate>j){starWidth='width:20%;';}else{var tempWidth=((data[i].rate-(j-1))*20<0?"0":(data[i].rate-(j-1))*20);starWidth='width:'+tempWidth+'%;';}
htmlReviews+='<span class="nr-icon nr-star gold" style="'+starWidth+'"></span>';}
htmlReviews+='        </div>'
+'        <div class="netreviews_reviews_rate">'+data[i].rate+'/5</div>'
+'    </div>'
+'    <p class="netreviews_customer_review">'+data[i].review+'</p>';if(data[i].medias!=undefined&&data[i].medias.length>0){var medias=JSON.parse(decodeURIComponent(atob(data[i].medias)));htmlReviews+='<ul class="netreviews_media_part">';for(var k=0;k<medias.length;k++){htmlReviews+='    <li>'
...
if(data[i].sign_helpful!=undefined){htmlReviews+='<p class="netreviews_helpful_block">'
...
if(data[i].moderation){data[i].moderation=data[i].moderation.reverse();for(var j=0;j<data[i].moderation.length;j++){htmlReviews+='<div class="netreviews_website_answer" style="display:'+(j==0?'block':'none')+'">'
+'    <p>'
+'        <span class="netreviews_answer_title"> &rsaquo;'
+'            Responder por&nbsp;';if(data[i].moderation[j].comment_origin==2){htmlReviews+=document.domain+'&nbsp;';}else if(data[i].moderation[j].comment_origin==3){if(data[i].firstname!='anonymous'&&data[i].lastname!='anonymous'&&data[i].firstname!='Anonymous'&&data[i].lastname!='Anonymous'){htmlReviews+=data[i].firstname+'&nbsp;'+(data[i].lastname?data[i].lastname[0]+'.':'')+'&nbsp;';}}else{htmlReviews+='Moderação&nbsp;';}
htmlReviews+=('pt'=='en'?'':'Publicado em')
+'            '+nrDateFormat(data[i].moderation[j].comment_date.substr(0,10),'d/m/Y')
+'        </span><br/>'
+data[i].moderation[j].comment
+'    </p>'
+'</div>';}
if(data[i].moderation.length>1){htmlReviews+='<a class="netreviews_button_comment show" href="javascript:void(0)" onclick="showComments(this);" style="display:block;">'
+'    <div class="nr-icon nr-comment"></div>'
+'    <span>Exibir a conversa</span>'
+'</a>'
+'<a class="netreviews_button_comment hide" href="javascript:void(0)" onclick="hideComments(this);" style="display:none;">'
+'    <div class="nr-icon nr-comment"></div>'
+'    <span>Esconder a conversa</span>'
+'</a>';}}
htmlReviews+='</div>';richSnippetsReviews+='{'
+'         "@type": "Review",'
+'         "reviewRating": {'
+'             "@type": "Rating",'
+'             "ratingValue": "'+data[i].rate+'"'
+'         },'
+'         "author": {'
+'             "@type": "Person",'
+'             "name": "'+data[i].firstname+'&nbsp;'+(data[i].lastname?data[i].lastname[0]+'.':'')+'"'
+'         },'
+'         "datePublished": "'+nrDateFormat(data[i].publish_date.substr(0,10),'d/m/Y')+'",'
+'         "reviewBody": "'+data[i].review+'"'
if(i<data.length-1){richSnippetsReviews+='},'}else{richSnippetsReviews+='}'}}
richSnippetsReviews+=']'
+' }'
+'</script>';if(($tagjs(".netreviews_review_part").length+data.length)<nbReviews){offset+=5;limit=5;htmlReviews+='<div id="netreviews_button_more_reviews" onclick="nrGetReviews('+JSON.stringify(rating)+','+offset+','+limit+','+nbReviews+');">'
+'    <div class="netreviews_button">'
+'        <span class="display">'
+"            Ver mais avaliações"
+'        </span>'
+'    </div>'
+'</div>';}}else if(nbReviews==0){htmlReviews='<p class="netreviews_no_reviews_block">'
+'    Não há avaliações para este produto.'
+'</p>';}else{console.log('TagJS Error, please contact Verified reviews');}
if(htmlReviews){$tagjs("#avisVerifiesAjaxImage").hide();$tagjs("#netreviews_button_more_reviews").remove();$tagjs(".netreviews_review_part").last().css("border-bottom","1px solid #e7e9ee");$tagjs("#netreviews_review_list").append(htmlReviews);$tagjs("#rich_snippets_json_ld").append(richSnippetsReviews);avLoadCookie();avDisplayVotes();}}
...
dataProps.product_id=$tagjs('.NETREVIEWS_PRODUCT_REVIEWS').data('productId')
dataProps.product_id=(dataProps.product_id!="xxxx"&&dataProps.product_id!=undefined)?dataProps.product_id:bodyParams.product;dataProps.product_rs_name=$tagjs('.NETREVIEWS_PRODUCT_REVIEWS').data('productName');dataProps.product_rs_name=(dataProps.product_rs_name!="xxxx"&&dataProps.product_rs_name!=undefined)?dataProps.product_rs_name:'';dataProps.product_rs_url=$tagjs('.NETREVIEWS_PRODUCT_REVIEWS').data('productUrl');dataProps.product_rs_url=(dataProps.product_rs_url!="xxxx"&&dataProps.product_rs_url!=undefined)?dataProps.product_rs_url:'';dataProps.product_rs_image=$tagjs('.NETREVIEWS_PRODUCT_REVIEWS').data('productImgUrl');dataProps.product_rs_image=(dataProps.product_rs_image!="xxxx"&&dataProps.product_rs_image!=undefined)?dataProps.product_rs_image:'';dataProps.product_rs_id=$tagjs('.NETREVIEWS_PRODUCT_REVIEWS').data('productRsId');dataProps.product_rs_id=(dataProps.product_rs_id!="xxxx"&&dataProps.product_rs_id!=undefined)?dataProps.product_rs_id:'';dataProps.product_price=$tagjs('.NETREVIEWS_PRODUCT_REVIEWS').data('productPrice');dataProps.product_price=(dataProps.product_price!="xxxx"&&dataProps.product_price!=undefined)?dataProps.product_price:'';return dataProps;}
...
function nrRateFilter(rating){var offset=0;var limit=5;var indexRateTotal=0;for(var i=0;i<rating.length;i++){indexRateTotal+=parseInt($tagjs('#netreviews_rate_total'+rating[i]).php());}
$tagjs('#netreviews_reviews_filter').val('newest');$tagjs('#netreviews_review_list').php('');$tagjs('#avisVerifiesAjaxImage').show();nrGetReviews(rating,offset,limit,indexRateTotal);}
